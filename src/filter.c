...
void *allocFilterMem( void ){
    return allocMatMem(FILTER_SIZ, FILTER_SIZ);
}
...
void genPascalNbs(mat *pascBuf){
    ...
    memsetMatrix(pascMat, 1, FILTER_SIZ, FILTER_SIZ);
...
void getScharrOperators(mat *hx, mat *hy, int *divFactor){
    ...
    cpyMatrix(hx, &scharrX[0], FILTER_SIZ, FILTER_SIZ);
    cpyMatrix(hy, &scharrY[0], FILTER_SIZ, FILTER_SIZ);
    *divFactor = 16;
}
...
void getSobelOperators(mat *hx, mat *hy, int *divFactor){
    ...
    cpyMatrix(hx, &sobelX[0], FILTER_SIZ, FILTER_SIZ);
    cpyMatrix(hy, &sobelY[0], FILTER_SIZ, FILTER_SIZ);
    *divFactor = 4;
}
